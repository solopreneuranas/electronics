...
router.post('/submit_product_details', upload.any(), function (req, res) {
    try {
        ...
        pool.query('insert into productdetails (productid, brandid, categoryid, modelno, description, color, price, offerprice, stock, status, hsncode, picture) values (?,?,?,?,?,?,?,?,?,?,?,?)',
            [req.body.productid, req.body.brandid, req.body.categoryid, req.body.modelno, req.body.description, req.body.color, req.body.price, req.body.offerprice, req.body.stock, req.body.status, req.body.hsncode, filenames + ''], function (error, result) {
                if (error) {
...
router.post('/update_product_details_pictures', upload.any(), function (req, res, next) {
    try {
        var filenames = req.files.map((item) => item.filename)
        pool.query('update productdetails set picture = ? where productdetailsid = ?', [filenames + '', req.body.productdetailsid], function (error, result) {
            if (error) {
                ...
                res.json({ status: true, message: 'Product Pictures updated successfully!' })
            }
        })
    }
    catch (e) {
